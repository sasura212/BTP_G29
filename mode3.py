"""
-------------------------------------------------------------------------
Mode 3: Power and Irms Dataset Generator
-------------------------------------------------------------------------
Iterates through duty cycles (D0, D1, D2) to generate Power and Irms values
for Mode 3 operational constraints.
...
print('Generating dataset for Mode 3... This might take a moment.')
...
for D0 in duty_cycles:
    for D1 in duty_cycles:
        for D2 in duty_cycles:
            ...
            # --- Constraint Checking for Mode 3 ---
            if (D1 < D0) and ((D0 + D2) > (1 + D1)) and ((D0 + D2) < 2):
                
                # --- Power Calculation for Mode 3 ---
                power = (V1*V2*T/L) * (
                    -1 + D0 - 0.5*D1 + 1.5*D2 - 
                    D0*D2 + 0.5*D1*D2 - 0.5*D2**2
                )
                
                # --- Irms Calculation for Mode 3 ---
...
if df.empty:
    ...
    # --- Save to CSV ---
    df.to_csv('mode3_dataset.csv', index=False)
    print('Dataset saved to mode3_dataset.csv')