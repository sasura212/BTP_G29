"""
-------------------------------------------------------------------------
Mode 6: Power and Irms Dataset Generator
-------------------------------------------------------------------------
Iterates through duty cycles (D0, D1, D2) to generate Power and Irms values
for Mode 6 operational constraints.
...
print('Generating dataset for Mode 6... This might take a moment.')
...
for D0 in duty_cycles:
    for D1 in duty_cycles:
        for D2 in duty_cycles:
            ...
            # --- Constraint Checking for Mode 6 ---
            if (D0 < D1) and ((D0 + D2) > 1) and ((D0 + D2) < (1 + D1)):
                
                # --- Power Calculation for Mode 6 ---
                power = (-(V1*V2*T)/L) * (
                    -0.5 + 0.5*D1 + 0.5*D2 - 0.5*D1*D2
                )
                
                # --- Irms Calculation for Mode 6 ---
...
if df.empty:
    ...
    # --- Save to CSV ---
    df.to_csv('mode6_dataset.csv', index=False)
    print('Dataset saved to mode6_dataset.csv')