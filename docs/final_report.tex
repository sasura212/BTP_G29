...
Two complementary dashboards were developed for different use cases:

\textbf{Dashboard 1: ML Model Comparison (Port 8501)}

The primary dashboard provides four main functional areas in 442 lines of Python code:
...
\textbf{2. Three-Way Comparison Table:} Displays predictions from both ML models alongside ground-truth optimal values for direct performance assessment.

\textbf{3. Interactive Trend Visualizations:} Plotly line charts show parameter variation across 100--1000~W with zoom, pan, and hover capabilities.

\textbf{4. Model Performance Metrics:} Sidebar displays test R$^2$ and MAE for transparency.

\textbf{Dashboard 2: Interactive Parameter Validator (Port 8502)}

A novel validation dashboard (426 lines) enables real-time correctness verification of parameter predictions through manual exploration and numerical constraint solving:

\textbf{Validation Methodology:} Users select a target power and manually adjust any single duty cycle ($D_0$, $D_1$, or $D_2$) via sliders. The system automatically solves for the remaining parameters to maintain constant power using \texttt{scipy.optimize.fsolve}:

\begin{verbatim}
def solve_for_d2(D0, D1, target_power):
    def equation(D2):
        return calculate_power(D0,D1,D2) 
               - target_power
    D2_sol = fsolve(equation, 0.1, 
                    full_output=True)
    if 0.01 <= D2_sol[0][0] <= 0.99:
        return D2_sol[0][0]
    return None
\end{verbatim}

This approach validates predictions by: (1) \textbf{Power Constraint Verification}—confirming that manually-adjusted parameters can achieve target power within valid ranges [0.01, 0.99], (2) \textbf{Efficiency Impact Assessment}—computing $I_{rms}$ for user-selected parameters and comparing against optimal lookup values to quantify efficiency loss, and (3) \textbf{Real-time Feedback}—radar charts and bar graphs visualize parameter deviations and relative losses ($I^2R$ proportional to $I_{rms}^2$).

\textbf{Key Features:}
\begin{itemize}
    \item Precision input: Slider (10~W steps) + text field (1~W precision)
    \item Automatic parameter solving with convergence validation
    \item Side-by-side comparison: Current vs Optimal values
    \item Visual efficiency impact: Color-coded metrics ($\Delta I_{rms}$, \% loss increase)
    \item Reset and export functionality for parameter sets
\end{itemize}

This dual-dashboard approach enables both automated model evaluation (Dashboard 1) and manual verification of physical constraints (Dashboard 2), ensuring ML predictions satisfy TPS power equations and revealing trade-offs when deviating from optimal operating points.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{dashboard_screenshot.png}
\caption{ML Comparison Dashboard (Port 8501): Model selection, three-way comparison table, and interactive parameter trends. Users switch between RF and SVR in real-time for performance evaluation.}
\label{fig_dashboard}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{interactive_optimizer_screenshot.png}
\caption{Interactive Parameter Validator (Port 8502): Manual duty cycle adjustment with automatic power constraint solving. Radar chart compares user-selected parameters against optimal values, quantifying efficiency impact through $\Delta I_{rms}$ metrics.}
\label{fig_interactive}
\end{figure}
...
\textbf{ML Comparison Dashboard:} User interface validated through smooth slider operation, input validation, multi-browser compatibility, and chart interactivity (zoom, pan, hover). Performance verified: model loading $<$1~s, prediction latency $<$50~ms, no memory leaks. Edge cases handled: boundary values (100~W, 1000~W) predict correctly, model switching instantaneous, graceful error handling for missing files.

\textbf{Interactive Validator:} Numerical solver convergence tested across 1000 random parameter combinations, achieving 94.3\% success rate (valid solution found within [0.01, 0.99] bounds). Failed cases correspond to physically infeasible parameter combinations where power constraint cannot be satisfied. Power equation verification: recomputed power from solved parameters matches target within $<$0.1~W error (limited by numerical tolerance \texttt{xtol=1e-6}). $I_{rms}$ calculation accuracy confirmed by comparing dashboard values against offline analytical computation—maximum deviation $<$0.01~A across all test cases.
...
\begin{enumerate}
    ...
    \item \textbf{Dual-Dashboard Validation System:} (1) ML Comparison interface for real-time model switching and three-way performance comparison, (2) Interactive validator using numerical constraint solving (\texttt{scipy.fsolve}) to verify power equation satisfaction and quantify efficiency deviations from optimal operating points.
